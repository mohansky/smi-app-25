...
const TimeFormatter = ({ timeString }: { timeString?: string }) => {
  ...
  return <span className="text-muted-foreground font-extralight font-mono text-[12px]">{timeString}</span>;
};
...
export const attendanceColumns: ColumnDef<AttendanceFormValues>[] = [
  {
    ...
    cell: ({ row }) => (
      <div className="flex flex-col items-start space-y-1">
        <DateFormatter dateString={row.getValue("date")} />
        <TimeFormatter timeString={row.original.time} />
      </div>
    ),
  },
  {
...
//         <DateFormatter dateString={row.getValue("date")} />
//         <Heading size="xxxs" fontweight="extralight">
//           <TimeFormatter timeString={row.getValue("time")} />
//         </Heading>
//       </div>
//     ),
//   },
